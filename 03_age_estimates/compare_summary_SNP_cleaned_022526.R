...
slope_group_2=compare_UKtwins_longt_select_2[compare_UKtwins_longt_select_2$scaffold == "SGB1860diffH_sweep_1_consensus", ]
slope_group_2$cluster_taxa.x='s__Bacteroides_faecis_SGB1860'
slope_group_2=merge(slope_group_2,compare_longt_UKtwins_summary_SGB_2,by='cluster_taxa.x')
predicted_values_SGB_2_slope_average=slope_group_2 %>% mutate(estimated_time=max_SNP.x/SNP*years) %>% group_by(scaffold.y) %>% 
  summarise(mean_estimated_time=mean(estimated_time),se_estimated_time=sd(estimated_time),n_estimated_time=n()) %>%
  mutate(se_estimated_time = if_else( n_estimated_time == 2, NA_real_, se_estimated_time))
...
compare_UKtwins_SGB_select=compare_UKtwins_SGB_select %>% mutate(estimated_time=max_SNP/SNP*years) %>% group_by(scaffold.y) %>% 
  summarise(mean_estimated_time=mean(estimated_time),se_estimated_time=sd(estimated_time),n_estimated_time=n()) %>%
  mutate(se_estimated_time = if_else( n_estimated_time == 2, NA_real_, se_estimated_time))
...
png('sweep_summary/tMRCA_longt_UKtwins_by_SGB_compare_log_2_e_SNP_updated_030226.png',res=300,width=4500,height=2500)
pcompare
dev.off()


ggsave('sweep_summary/tMRCA_longt_UKtwins_by_SGB_compare_log_2_e_SNP_updated_030226.svg',plot=pcompare,
