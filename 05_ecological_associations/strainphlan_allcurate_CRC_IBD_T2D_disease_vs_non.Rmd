...
sweep_list_all=read.csv('confirmed_sweep_from_pairwise.csv')
df_strainphlan_cutoff=read.csv('strainphlan_cutoff_new.csv')
...
for (i in 1: length(folder_list_all)){
  ...
  total_sweep=lapply(sweep, function(x) read.table(paste0("compare_select_lists/",x,"_sweep.txt"),header = F))
...
new_issweep_p_df_unmerged_all$disease=as.factor(new_issweep_p_df_unmerged_all$disease)
...
ggsave('volcano_diseases_2.svg',plot=pv, width=1400,height = 1600,units='px')
...
disease_match=c("non-target"=disease_cols[1],"CRC"=disease_cols[2],'T2D'=disease_cols[3],'advanced age'=disease_cols[4],'CD'=disease_cols[5],'UC'=disease_cols[6])
...
habitat_total_2=ggplot(newtotalsweep_merged_list_all_SGBs_select_n, aes(y = disease_sweep_colored, x = n_norm, fill = disease.x)) +
  ...
  geom_bar(position = "fill",stat = "identity",
           aes(colour = after_scale(fill)),   # <- inside aes()
    linewidth = 0.3,
    linejoin = "mitre",
    lineend  = "square") +
  scale_fill_manual(values=disease_cols)+
...
png('habitat_total_sweep.png',res=300,width = 5000,height = 4500)
habitat_total_2
dev.off()

ggsave("habitat_total_sweep.svg", plot=habitat_total_2, width = 5000, height = 5000,units='px')
...
habitat_BU_2=ggplot(newtotalsweep_merged_list_all_SGBs_select_n_BU, aes(y = disease_sweep_colored, x = n_norm, fill = disease.x)) +
  ...
  scale_fill_manual(values=disease_cols)+
  scale_x_continuous(expand = c(0,0),labels = scales::percent_format()) +
  theme(axis.text.y.left = ggtext::element_markdown(size = 14),
...
ggsave('habitat_BU_sweep.svg',plot=habitat_BU_2, width = 2500,height = 1000,units='px')
...
isolate_sweeps_table=Sys.glob('confirmed_select_lists/*.txt')

isolate_sweeps_list=lapply(isolate_sweeps_table, function(x) read.table(x))
names(isolate_sweeps_list)=unlist(lapply(isolate_sweeps_table,function(x) gsub('confirmed_select_lists/','',x)))
...
unique_genes=read.csv('sweep_differentiating_genes_clean.csv')
positive_genes=read.csv('genes_under_positive_selection.csv')
