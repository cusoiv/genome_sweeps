...
cp ${HOMEFOLDER}/CoreSimul/mycontrol_64_pop5.txt ${THISWORKFOLDER}
cp ${HOMEFOLDER}/CoreSimul/mytree_64_pop5.nwk ${THISWORKFOLDER}
cp ${HOMEFOLDER}/CoreSimul/ref.fa ${THISWORKFOLDER}

#edit control.txt according to input paramters
sed -i "s|.SLURM_ARRAY_TASK_ID|$SLURM_ARRAY_TASK_ID|g" mycontrol_64_pop5.txt
sed -i "s|.rho|$rho|g" mycontrol_64_pop5.txt
sed -i "s|.scale|$scale|g" mycontrol_64_pop5.txt
sed -i "s|.delta|$delta|g" mycontrol_64_pop5.txt
sed -i "s|.coeff|$coeff|g" mycontrol_64_pop5.txt

#run analysis
module load python3
python3 coresimul_master.py mycontrol_64_pop5.txt

#copy back to Results
mkdir -p ${HOMEFOLDER}/../Results/simulated_evolution/coresimul_64_pop5

rsync -aR ././test*/* ${HOMEFOLDER}/../Results/simulated_evolution/coresimul_64_pop5
