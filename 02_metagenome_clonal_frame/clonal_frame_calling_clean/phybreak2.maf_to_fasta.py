...
MSA_name_keepgap = output_prefix+".core.keepgap.fasta"
...
seqdict_json=json.dumps(seqdict)
f= open('seqdict.json','w')
f.write(seqdict_json)
f.close()
...
#write all aligned contigs without degapped seqs
os.makedirs(input_dir+"core_contigs",exist_ok=True)
for label in use_labels:
        corefile_withgap_contig = open(input_dir+ "core_contigs/"+ str(label)+ ".fasta","w")
        for iso in seqdict[label].keys():
            corefile_withgap_contig.write(">"+iso +"\n"+ seqdict[label][iso] +"\n")
        corefile_withgap_contig.close()

#write alignment without degapped sequences

#build dictionary

keepgap_seqdict = {}
keepgap_seqdict_contig = {}
for iso in isolist:  #so this removes the extremely gapped columns but not the partially gapped ones
        for i in range(0,len(ref_order)):
                label = ref_order[i][1]
                start = ref_order[i][0]
                if label in use_labels:
                        seq = seqdict[label][iso]
                        try:
                                keepgap_seqdict[iso] += seq
                        except:
                                keepgap_seqdict[iso] = seq

corefile_withgap = open(input_dir+MSA_name_keepgap,"w")
#corefile_withgap = open ("testmaf.core.keepgap.fasta","w")
for k in range(0,len(isolist)):
        iso = isolist[k]
        corefile_withgap.write(">"+iso +"\n"+ keepgap_seqdict[iso] +"\n")
corefile_withgap.close()
