import os
#os.chdir("C:/Users/Xiaoqian/Desktop/PopCOGenT/clonal_frame_calling_seqseqpan/")
...
consensus_core = output_prefix+".core.consensus.fasta"
...
def find_consensus_seq(msa_dict):
	...
	for i in nt_dict:
		...
		for l in 'ATGC':
			sn=sn+nt_dict[i][l]
		if sn>0:
			tmp_dict= { nt: nt_dict[i][nt] for nt in 'ATGC' }
			consensus_seq=consensus_seq+max(tmp_dict,key=tmp_dict.get)
		else:
...
msa = open(input_dir+MSA_name_keepgap,"r")
